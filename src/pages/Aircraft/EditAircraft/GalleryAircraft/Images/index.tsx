...
// services
import AircraftImageService from "../../../../../services/aircraft-image.service";
...
{
    const { aircraft_id, type, images=[] } = props;
    const aircraftImageService = new AircraftImageService();
    ...
    async function index()
    {
        ...
        try {
            const response = await aircraftImageService.pagination(paginationFilter);
    ...
    async function handleDelete()
    {
        ...
        for (let index = 0; index < selecteds.length; index++) {
            await aircraftImageService.delete(selecteds[index]);
    ...
    async function handleDoc(data: AircraftImage, checked: boolean)
    {
        data.use_in_document = checked;
        
        try {
            await aircraftImageService.update(data);
        } catch (error) {
            console.error(error);
        }
    }
    ...
    return(
        <ListImagesView>
            <Upload 
                open={open} 
                fileName="file" 
                requestUrl="/AircraftImages"
                onUploaded={handleUploaded} 
                onCancel={handleClose} 
                params={{ view, type, aircraft_id }} 
            />
            <ListImagesHeader>
                {selecteds.length > 0 ? (
                    <SelectedItemsHeader>
